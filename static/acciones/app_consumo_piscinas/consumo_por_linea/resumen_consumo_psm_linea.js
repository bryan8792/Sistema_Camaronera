...
function generate_report_piscinas() {
    ...
    tb_piscinas_por_insumos = $('#insumos_conglomerado_psm').DataTable({
        ...
        columns: [
            {"data": "producto_empresa.nombre_prod.nombre","width": "20%"},
        ...
        initComplete: function (settings, json) {
            ...
            json.map(function (valor, indice) {
                ...
                if (movimientos_encontrados.indexOf(valor.producto_empresa.nombre_prod.descripcion) === -1) {
                    movimientos_encontrados.push(valor.producto_empresa.nombre_prod.descripcion);
                    nuevo_ArrayObject.push(valor);
                } else {
                    var recuperado = movimientos_encontrados.indexOf(valor.producto_empresa.nombre_prod.descripcion);
            ...
            nuevo_ArrayObject.map(function (valor, indice) {
                ...
                table+='<tr>'
                table+='<td scope="col" style="width: 20%; text-align: left">'+ valor.producto_empresa.nombre_prod.categoria.nombre +'</td>'
                table+='<td scope="col" style="width: 50%; text-align: left" >'+ valor.producto_empresa.nombre_prod.descripcion +'</td>'
                table+='<td scope="col" style="width: 10%; text-align: center">'+ cantidad.toFixed(2) +'</td>'
                table+='<td scope="col" style="width: 10%; text-align: center">'+ costo.toFixed(10) +'</td>'
                table+='<td scope="col" style="width: 10%; text-align: center">'+ total_consumos.toFixed(2) +'</td>'
                table+='</tr>';
                acum1 += cantidad;  acum2 += total_consumos;

            });
                table+='<tr>'
                table+='<th colspan="2" scope="col" style="width: 50%; text-align: center"> Cantidad Total</th>'
            ...
            // document.getElementById("insumos_conglomerado_psm").innerHTML = table2;
        }
    });
}

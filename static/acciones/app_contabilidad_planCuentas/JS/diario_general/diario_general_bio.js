        ...
        buttons: [
            {
                ...
                title: "DIARIO GENERAL ACUMULADO", // Add this line
                exportOptions: {
                    format: {
            ...
            },
            {
                ...
                title: "DIARIO GENERAL ACUMULADO",
                exportOptions: {
                    format: {
                        body: (data, row, column, node) => {
                            if (row.tipo === "total" || row.tipo === "grand_total") {
            ...
            },
            {
                ...
                customize: (doc) => {
                    ...
                    // Get the date range
                    let startDate = date_range ? date_range.startDate.format("DD/MM/YYYY") : moment(date_now).format("DD/MM/YYYY")
                    let endDate = date_range ? date_range.endDate.format("DD/MM/YYYY") : moment(date_now).format("DD/MM/YYYY")

                    // Get current date and time
                    let currentDateTime = moment().format("DD/MM/YYYY HH:mm:ss")

                    // Replace the title and add date information
                    if (doc.content[0] && doc.content[0].text) {
                        doc.content[0].text = "DIARIO GENERAL ACUMULADO EMPRESA BIO";
                    ...
                    doc.content.splice(2, 0, {
                        text: `Generado el: ${currentDateTime}`,
                        alignment: "right",
                        margin: [0, 0, 0, 10],
                        fontSize: 10
                    });
                    ...
                    doc.content[3].layout = objLayout  // Update this to 3 since we added 2 new elements
                },
            },
            {
                extend: "print",
                ...
                titleAttr: "Imprimir",
                className: "btn btn-info btn-sm",
                title: '', // Set to empty to remove default title
                exportOptions: {
                    format: {
                        body: (data, row, column, node) => {
                            if (row.tipo === "total" || row.tipo === "grand_total") {
                ...
                customize: function (win) {
                    ...
                    let header = $(win.document.body).find('h2');
                    if (header.length) {
                        header.html('DIARIO GENERAL ACUMULADO');
                    } else {
                        $(win.document.body).prepend('<h2 class="text-center" style="text-align:center;margin-bottom:10px;">DIARIO GENERAL ACUMULADO</h2>');
                    }

                    $(win.document.body).find('h1').after(
                        '<div style="text-align:center;margin-bottom:15px;">' +
                        '<p><strong>Empresa:</strong> ' + empresa + '</p>' +
                        '<p><strong>Del:</strong> ' + startDate + ' al ' + endDate + '</p>' +
                        '<p><strong>Generado el:</strong> ' + currentDateTime + '</p>' +
                        '</div>'
                    );

...
// === AJAX handlers for ATS and Comprobante save buttons ===
(function ($) {
    function getCookie(name) {
        let cookieValue = null;
        if (document.cookie && document.cookie !== '') {
...
// ✅ Guardar ATS
    ...
    // Comprobante save button
    $(document).on('click', '.btn-save-comprobante', function (e) {
        e.preventDefault();
        const $btn = $(this);
        ...
        $.ajax({
            ...
            Swal.fire({icon: 'success', title: 'Guardado', text: 'Comprobante guardado correctamente.'});
            // if response contains redirect or print_url, handle it
            if (resp && resp.print_url) {
                // offer to open print in new tab
                Swal.fire({
            ...
            let msg = 'Ocurrió un error al guardar el comprobante.';
